...
clean_cluster()
{
    ...
    ${PSSH} -h ${CLUSTER_HOSTS}  -i "rm -rf ${DATA_DIR}; mkdir -p ${WTF_COORDINATOR_DATA_DIR} \
        && mkdir ${WTF_DAEMON_DATA_DIR}; mkdir ${HYPERDEX_COORDINATOR_DATA_DIR} \
        ; mkdir ${HYPERDEX_DAEMON_DATA_DIR}; rm wtf-*.log; mkdir ${WTF_DAEMON_DATA_DIR}/data"
}
...
reset_cluster()
{
    ...
    ssh ${HC} "echo 'space wtf key path attributes string blockmap, int directory, int mode, string owner, string group' | ${HYPERDEX} add-space -h ${HC} -p ${HYPERDEX_PORT}"
    sleep 5
