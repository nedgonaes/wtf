...
clean_cluster()
{
    ...
    ${PSSH} -h ${CLUSTER_HOSTS} -i "rm -rf ${DATA_DIR}"
    ${PSSH} -h ${CLUSTER_HOSTS} -i "mkdir -p ${WTF_COORDINATOR_DATA_DIR} \
        && mkdir ${WTF_DAEMON_DATA_DIR}"
    ${PSSH} -h ${CLUSTER_HOSTS} -i "mkdir ${HYPERDEX_COORDINATOR_DATA_DIR}"
    ${PSSH} -h ${CLUSTER_HOSTS} -i "mkdir ${HYPERDEX_DAEMON_DATA_DIR}"
    ${PSSH} -h ${CLUSTER_HOSTS} -i "rm wtf-*.log"
    ${PSSH} -h ${CLUSTER_HOSTS} -i "mkdir ${WTF_DAEMON_DATA_DIR}/data"
}
...
reset_cluster()
{
    ...
    ssh ${HC} "${HYPERDEX} coordinator -D ${HYPERDEX_COORDINATOR_DATA_DIR} -l ${HC} -p ${HYPERDEX_PORT}"
    sleep 5
